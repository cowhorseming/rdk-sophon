...
function parseArgs(argv) {
	...
	for (let i = 0; i < argv.length; i++) {
		...
		if (value === "--config") options.config = resolve(argv[++i]);
			...
			console.log(`用法：node benchmark-openai-compatible.mjs [选项]

选项：
  --config <path>       Pi models.json 路径
  --provider <id>       提供方标识（默认：amd）
  --runs <count>        计分请求次数（默认：3）
  --prompt <text>       固定基准提示词
  --output <path>       同时写入脱敏后的 JSON 报告
`);
			process.exit(0);
