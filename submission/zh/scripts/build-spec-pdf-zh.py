...
class LocalizedCanvas(canvas.Canvas):
    def setFont(self, psfontname, size, leading=None):
        mapped = FONT_BOLD if psfontname == "Helvetica-Bold" else FONT_REGULAR if psfontname == "Helvetica" else psfontname
        return super().setFont(mapped, size, leading)

    def drawString(self, x, y, text, *args, **kwargs):
        return super().drawString(x, y, tr(text), *args, **kwargs)

    def drawRightString(self, x, y, text, *args, **kwargs):
        return super().drawRightString(x, y, tr(text), *args, **kwargs)

    def drawCentredString(self, x, y, text, *args, **kwargs):
        return super().drawCentredString(x, y, tr(text), *args, **kwargs)
...
def paragraph(c, text, x, y_top, width, *, size=10.5, leading=15, color=INK, bold=False):
    item = Paragraph(tr(text), pstyle(size=size, leading=leading, color=color, bold=bold))
...
def page_header(c, kicker, title, subtitle=None):
    kicker = tr(kicker)
    title = tr(title)
    subtitle = tr(subtitle) if subtitle else None
    c.setFillColor(BLUE)
    ...
    title_size = min(26, max(18, 26 * title_width / stringWidth(title, FONT_BOLD, 26)))
...
def label(c, text, x, y, *, fill=BLUE, width=None):
    ...
    text_w = stringWidth(text, FONT_BOLD, 7.5)
...
def build():
    ...
    c = LocalizedCanvas(str(OUTPUT), pagesize=A4, pageCompression=1)
    c.setTitle(tr("RDK Agent - Track 2 Project Specification"))
    c.setAuthor(tr("RDK Agent Team"))
    c.setSubject(tr("AMD Radeon AI Agent Hackathon 2026 - Track 2"))
    ...
    paragraph(
        c,
        "Official repository: github.com/AMD-DEV-CONTEST/Radeon-hackathon-2026-07<br/>Event page: luma.com/amd-4dhi<br/>Project evidence: submission/zh/evidence/verification-2026-08-05.md",
        42,
        107,
        PAGE_W - 84,
        size=8.2,
