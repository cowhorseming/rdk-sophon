...
- 首轮先写能执行的最小测试：功能尚未实现时，因缺少目标功能而失败是有效红测；功能已存在时，绿色回归测试同样可交付。
...
export class AgentPromptBuilder {
	build(request: AgentRunRequest): string {
		const history = this.deliveryHistory(request.previousDeliveries);
	...
	private deliveryHistory(deliveries: readonly Delivery[]): string {
		if (deliveries.length === 0) return "无：你是流水线的第一个 Agent。";
		return deliveries
			.map((delivery) => `### ${delivery.stageId}\n${delivery.summary.slice(-maxPriorDeliveryCharacters)}`)
			.join("\n\n");
	}

	private resultContract(expectation: AgentExpectation): string {
